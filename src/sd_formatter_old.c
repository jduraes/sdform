...
int sd_formatter_show_card_content(void) {
    ...
    if (partition_count > 0) {
        ...
        // Auto-browse FAT32 partitions
        for (int i = 0; i < partition_count; i++) {
            if (strcmp(partitions[i].filesystem, "FAT32") == 0) {
                printf("\n=== PARTITION %d CONTENTS ===\n", i + 1);
                sd_analyzer_list_fat_directory(partitions[i].start_lba, "/");
                break; // Only show first FAT32 partition
            }
        }
    } else {
